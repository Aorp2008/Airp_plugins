...
async def lottery_end():
    filename = "config_lottery.txt"
    if not os.path.exists(filename):
        with open(filename, "w") as file:
            file.write("") ##请设置指定中奖的用户id或者更改这个文件一行一个用户
    ...
    if all_user:
        ...
        with open(filename, "r") as file:
            config_user_ids = [line.strip() for line in file.readlines()]
        win_user.extend(config_user_ids)
        if len(win_user) < win_user_num:
            num_remaining = win_user_num - len(win_user)
            remaining_user = [user_id for user_id in all_user if user_id not in win_user]
            if remaining_user:
                random_remaining = secret_generator.sample(remaining_user, num_remaining)
                win_user.extend(random_remaining)
    win_text = (
        end_text.format(
            lottery_json["title"],
